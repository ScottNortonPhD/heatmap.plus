    ...
    verbose = getOption("verbose"), ...)
{
    ...
    if (!missing(RowSideColors)) {
      ...
      rsc <- matrix(as.numeric(rsc), nrow=dim(RowSideColors)[1])
        image(t(rsc), col = as.vector(rsc.colors), axes = FALSE)

        if (length(colnames(RowSideColors))>0) {
          if (length(colnames(RowSideColors))==1) {
            axis(1, 0, colnames(RowSideColors), las=2, tick=FALSE)
          } else {
    ...
    if (!missing(ColSideColors)) {
        ...
        csc <- matrix(as.numeric(csc), nrow=dim(ColSideColors)[1])
        image(csc, col = as.vector(csc.colors), axes = FALSE)

        if (length(colnames(ColSideColors))>0) {
          if (length(colnames(ColSideColors))==1) {
            axis(1, 0, colnames(ColSideColors), las=2, tick=FALSE)
          } else {
